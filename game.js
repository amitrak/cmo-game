...
const CONFLICTS = [
  {
    ...
    text: 'An internal chat message leaks. An employee referred to customers as "walking wallets." #Boycott{product} is trending.',
    choices: [
  ...
  },
  {
    ...
    text: 'It\'s the end of the quarter. Sales is at 82% of target. The CEO, CFO, and VP Sales are in your office. "We need you to go all-in on performance marketing. Run a 40% discount. NOW." The CFO is literally sweating.',
    choices: [
  ...
  },
  {
    ...
    text: 'Your top influencer partner just posted a 40-minute video "proving" birds are government drones. {product} is front and center. 2 million views and climbing.',
    choices: [
  ...
  },
  {
    ...
    text: 'Your top-performing ad channel just hit a wall. CPMs tripled overnight. Your $50k monthly budget is delivering $15k in results.',
    choices: [
  ...
  },
  {
    ...
    text: 'BREAKING: A-list celebrity was photographed using your {product} at Soho House! Paparazzi photos are everywhere. Your social mentions just 10x\'d. Your inbox is overflowing.',
    choices: [
  ...
  },
  {
    ...
    choices: [
      { text: 'Pivot to short-form video ($15k production)', cost: 15000, brandEquity: 2, revMult: 0.9, ceoPat: 0, luck: [0.55, { brandEquity: 8, revMult: 1.1, ceoPat: 10, override: 'The pivot pays off. Your video strategy gets MORE reach than the old approach. The algorithm change was a gift in disguise.' }], outcome: 'The pivot takes time. Your first vertical videos crop your product out of frame. Quality improves by week 3, but you\'ve lost momentum. Marketing lesson: Platform dependency is a strategic risk. Always have a Plan B.' },
      { text: 'Shift budget from organic to paid social', cost: 15000, brandEquity: -6, revMult: 1.0, ceoPat: 5, outcome: 'Your reach rebounds, though you\'re buying what you used to get for free. Marketing lesson: The platforms always win. Always.' },
      { text: 'Invest in owned channels: email, web, community ($25k)', cost: 25000, brandEquity: 8, revMult: 0.85, ceoPat: -5, futureRevBonus: 0.10, outcome: 'Revenue dips but you\'re building on land you OWN. Email list grows. SEO climbs. Community hits 5,000 members who actually care. Marketing lesson: Rented audiences are rented. Build on owned land.' },
  ...
  },
  {
    ...
    text: 'A VC-backed competitor just launched a clone of {product} that\'s 20% cheaper. They\'re running side-by-side comparison ads. Their tagline? "Like {name}, but better." They have $25M in funding. You have anxiety.',
    choices: [
      { text: 'Double down on your brand ($30k)', cost: 30000, brandEquity: 7, revMult: 0.95, ceoPat: 5, outcome: 'Customers recognize {product} as the original. Your "why" resonates deeper than their "what" features. Marketing lesson: Brand storytelling is one of the strongest competitive moats.' },
      ...
      { text: 'Sue for trademark/trade dress infringement ($50k)', cost: 50000, brandEquity: 0, revMult: 0.9, ceoPat: -5, luck: [0.3, { brandEquity: 6, revMult: 1.1, ceoPat: 15, override: 'The competitor is spooked! They back down and pull their ads. The press coverage frames you as the original innovator. Sometimes a well-timed legal win generates more press than any ad campaign could.', cost: -20000 }], outcome: 'Legal drags on with no end in sight. Meanwhile, they keep selling. The judge says it\'s not similar enough to warrant an injunction. You\'re out $50k with nothing to show for it. Marketing lesson: Litigation is not a marketing strategy.' },
  ...
  },
  {
    ...
    text: 'Your payment processor was hacked. Customer emails and purchase history may have been exposed. A security researcher just tweeted about it. Media is reaching out for comment. Legal is panicking.',
  ...
  },
  {
    ...
    choices: [
      ...
      { text: 'Go AGGRESSIVE: "Buy when there\'s blood in the streets" ($50k extra)', cost: 50000, brandEquity: 5, revMult: 0.85, ceoPat: -15, luck: [0.4, { brandEquity: 9, revMult: 1.3, ceoPat: 15, override: 'LEGENDARY MOVE. Cheap ad inventory, massive reach, and when the market rebounds, you EXPLODE. Case studies will be written about this.' }], outcome: 'You burn through cash during a downturn. The board is apoplectic. Revenue doesn\'t spike because consumer spending is DOWN regardless of your ad spend. Marketing lesson: You can\'t advertise your way out of a recession.' }
    ]
  },
  {
  ...
  },
  {
    ...
    text: 'Something beautiful is happening. Without any paid effort, a community of {product} fans has formed. They have forums (12k members), a community server (8k), and they\'re creating memes, fan art, and unboxing videos.',
    choices: [
  ...
  },
  {
    ...
    condition: (g) => (g.product !== 'software' || g.positioning === 'consumer' || g.positioning === 'smb') && g.launchTactics.includes('influencer'),
    textFn: (g) => 'You paid a Gen Z influencer to promote {product}. They just posted the video. They mispronounced the brand name' + ((g.positioning === 'consumer' || g.positioning === 'smb') ? ' and a competitor\'s app is visible on their phone.' : ', the logo is backward, and you can clearly see a competitor\'s product on their desk.') + ' The comments are brutal.',
    choices: [
...
const HOLIDAY_EVENT = {
  ...
  strategies: [
    { name: 'Brand-Led Holiday Campaign', cost: 50000, brandMult: 1.0, perfMult: 0.3, desc: 'Emotional storytelling. "The holidays are about..." You know the type. Polar bear vibes.', icon: '🎅' },
    ...
    { name: 'Charity Tie-In', cost: 5000, brandMult: 0.2, perfMult: 0.05, desc: 'Align your brand with a cause and earn feel-good press.', icon: '❤️' },
...
function shuffleConflicts() {
  ...
  // Weighted shuffle: conflicts with weight functions get probability adjustments
  function weightedShuffle(arr) {
    const weighted = arr.map(c => ({ c, w: c.weight ? c.weight(G) : 1 }));
    const result = [];
    while (weighted.length > 0) {
      ...
      for (let i = 0; i < weighted.length; i++) {
        r -= weighted[i].w;
        if (r <= 0) { result.push(weighted[i].c); weighted.splice(i, 1); break; }
      }
    }
    return result;
  }
  crises = weightedShuffle(crises);
  positive = weightedShuffle(positive);
  pressure = weightedShuffle(pressure);
  market = weightedShuffle(market);
  ...
  // Quarter-end crunch should land on month 3 (index 1) or month 9 (index 6)
  const crunchIdx = selected.findIndex(c => c.id === 'quarter_crunch');
  if (crunchIdx !== -1 && crunchIdx !== 1 && crunchIdx !== 6) {
    const targetIdx = Math.random() < 0.5 ? 1 : 6;
    [selected[crunchIdx], selected[targetIdx]] = [selected[targetIdx], selected[crunchIdx]];
  }
...
function applyConflictChoice(conflictIdx, choiceIdx) {
  ...
  if (choice.luck) {
    ...
    // Hockey stick: data team affects luck (skip = much worse, agency = slightly worse)
    if (conflict.id === 'hockey_stick') {
      if (G.team.data === 'skip') chance = Math.max(0, chance * 0.3);
      else if (G.team.data === 'agency') chance = Math.max(0, chance * 0.7);
    }
    // Data breach: website tier affects luck (cheap = unlucky outcomes more likely, premium = lucky outcomes more likely)
    if (conflict.id === 'data_breach') {
      if (G.siteTier === 'template') chance = Math.max(0, chance * 0.3);
      else if (G.siteTier === 'custom') chance = Math.max(0, chance * 0.7);
      else if (G.siteTier === 'premium') chance = Math.min(1, chance * 1.4);
      else if (G.siteTier === 'enterprise') chance = Math.min(1, chance * 1.8);
    }
  ...
  // Quarter crunch: value positioning doesn't take brand hit from discounting (it's on-brand)
  if (conflict.id === 'quarter_crunch' && (choiceIdx === 0 || choiceIdx === 2) && G.positioning === 'value') {
    effects.brandEquity = 0;
  }
...
function renderConflict() {
  ...
  let text = (conflict.textFn ? conflict.textFn(G) : conflict.text)
...
function renderHolidayAllocation() {
  ...
  return `<div class="screen">
    ...
    <div class="narrative">
      ...
      <p>${(['enterprise', 'government', 'smb'].includes(G.positioning)) ? 'It\'s the end of the fiscal year and time for one final push. Brand equity determines your holiday multiplier.' : 'Black Friday. Cyber Monday. White elephants. End the year with a bang. Brand equity determines your holiday multiplier.'}</p>
